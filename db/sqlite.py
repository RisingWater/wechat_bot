...
def get_all_processors(db_path: str = "database.db"):
    """获取所有处理器，用于下拉菜单"""
    
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    
    cursor.execute("SELECT processor_id, processor_name, description FROM processors ORDER BY processor_name")
    ...
    # 转换为字典列表格式，方便前端使用
    result = []
    for processor_id, processor_name, description in processors:
        result.append({
            "id": processor_id,
            "name": processor_name,
